        ...
        <main className="max-w-5xl mx-auto px-4 py-6 sm:px-6 lg:px-8 pt-28">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-gray-800 dark:text-white mb-1">Manage Courses</h1>
                <p className="text-sm text-gray-600 dark:text-gray-300">Create, edit, or remove courses here.</p>
              </div>
              <div>
                <button onClick={openCreate} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition">+ Create New Course</button>
              </div>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-900">
                  <tr>
                    <th className="px-4 py-2 text-left text-sm font-medium text-gray-500 dark:text-gray-300">ID</th>
                    <th className="px-4 py-2 text-left text-sm font-medium text-gray-500 dark:text-gray-300">Title</th>
                    <th className="px-4 py-2 text-left text-sm font-medium text-gray-500 dark:text-gray-300">Instructor</th>
                    <th className="px-4 py-2 text-left text-sm font-medium text-gray-500 dark:text-gray-300">Capacity</th>
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                  {loading ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-300">Loading courses...</td>
                    </tr>
                  ) : error ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-6 text-center text-sm text-red-600 dark:text-red-400">Error: {error}</td>
                    </tr>
                  ) : courses.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-300">No courses yet. Create one.</td>
                    </tr>
                  ) : courses.map(course => (
                    <tr key={course.id || course.course_id}>
                      <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{course.id || course.course_id}</td>
                      <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{course.course_title || course.title}</td>
                      <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{course.instructor}</td>
                      <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-200">{course.capacity || course.seats}</td>
                      <td className="px-4 py-3 text-sm text-right">
                        <button onClick={() => openEdit(course)} className="mr-2 px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700">Edit</button>
                        <button onClick={() => handleDelete(course.id)} className="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700">Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              ...
              <form onSubmit={handleSubmit} className="relative bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md mx-4 p-6 z-10">
                ...
                <div className="flex justify-end">
                  ...
                  <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded">{editingCourse ? 'Save' : 'Create'}</button>
                </div>
              </form>
            </div>
          )}
