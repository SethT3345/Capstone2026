...
export default function EnrolledCourses({ cardComponent: Card = MyLearningCard, children, gridClass = 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6' }) {
  ...
  if (loading) return <div className="text-center text-gray-600 dark:text-gray-300">Loading courses...</div>
  if (error) return <div className="text-center text-red-600">{error}</div>
  if (!courses || courses.length === 0) return <div className="text-center text-gray-600 dark:text-gray-300">You are not enrolled in any courses yet.</div>
